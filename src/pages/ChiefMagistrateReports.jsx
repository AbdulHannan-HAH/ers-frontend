...
export default function ChiefMagistrateReports() {
  ...
  const fetchReports = async () => {
    try {
      const res = await axios.get(
        `https://ers-backend-f.onrender.com/api/magistrate-reports/chief/all?court=${encodeURIComponent(selectedCourt)}`,
        {
  ...
  const markAsViewed = async (id) => {
    try {
      await axios.patch(`https://ers-backend-f.onrender.com/api/magistrate-reports/chief/view/${id}`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const confirmReject = async () => {
    try {
      await axios.patch(
        `https://ers-backend-f.onrender.com/api/magistrate-reports/chief/reject/${rejectModal.reportId}`,
