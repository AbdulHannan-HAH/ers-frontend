...
export default function ChiefReports() {
  ...
  const fetchReports = async () => {
    try {
      const res = await axios.get(
        `https://ers-backend-f.onrender.com/api/returns/chief/all?court=${encodeURIComponent(selectedCourt)}`,
  ...
  const markAsViewed = async (id) => {
    try {
      await axios.patch(`https://ers-backend-f.onrender.com/api/returns/chief/view/${id}`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const confirmReject = async () => {
    try {
      await axios.patch(
        `https://ers-backend-f.onrender.com/api/returns/chief/reject/${selectedReport._id}`,
  ...
  validReports.forEach((report, index) => {
    ...
    if (criminalCases.length > 0) {
      doc.setFont("helvetica", "bold");
      doc.setFontSize(12);
      doc.text("SUMMARY CASE REPORTING FOR CRIMINAL CASES:", doc.internal.pageSize.width / 2, y, { align: "center" });
      y += 8;
    ...
    if (civilCases.length > 0) {
      doc.setFont("helvetica", "bold");
      doc.setFontSize(12);
      doc.text("SUMMARY CASE REPORTING FOR CIVIL CASES:", doc.internal.pageSize.width / 2, y, { align: "center" });
      y += 8;
